...
public class ConfigurationServiceImpl implements ConfigurationService{
    ...
    public ResponseEntity<ConfigurationDTO> createConfiguration(CreateConfigurationDTO createConfigurationDTO) {
        Optional<Configuration> configuration = this.configurationRepository.findByUserId(createConfigurationDTO.userId);
        if (configuration.isPresent()) {
            return new ResponseEntity<>(null, HttpStatus.CONFLICT);
        }

        try {
            this.restTemplate.postForObject(assetServiceUrl + "configuration-" + createConfigurationDTO.userId.toString() + "-" + createConfigurationDTO.name, createConfigurationDTO.content, String.class);
        } catch (Exception e) {
    ...
    public ResponseEntity<String> deleteConfiguration(Long userId) {
        try {
            ...
            try {
                this.restTemplate.delete(assetServiceUrl + "configuration-" + userId.toString() + "-" + configuration.getName());
